        ...
        for precision in "${precisions[@]}"; do
            for size in "${sizes[@]}"; do  
                ...
                tmux new-session -d -s measure-session "while true; 
                    do nvidia-smi --query-gpu=utilization.gpu,utilization.memory,memory.total,memory.free,memory.used --format=csv >> $savePath/$type$model$device.log; sleep 0.1; 
                    done" &
                ...
                cd build; 
                ./TFLiteMemory $pathTFLite/models/$type/$model $precision $pathTFLite/data/$image $threads $device $verbose;
                cd ..;
...
tmux capture-pane -pS - > /project/pane-history.log;